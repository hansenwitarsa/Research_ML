# sama kayak model 3 cuma tambain dense layer
...
class TrajectoryPredictionModel(tf.keras.Model):
    ...
    def call(self, input):
        ...
        for i in range(gcn_output.shape[0]):
            ...
            # 1st Dense Layer
            Dense1 = self.Dense(temporal)
            # Dense1 shape (64, output_dense=24)
            Dense1_reshaped = tf.reshape(Dense1, (-1, 12, 2))
            
            decode = self.decoder(Dense1_reshaped)
